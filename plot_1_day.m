function R = plot_1_day(day, prec_threshold, lightning_treshold)
...
for hour = 1:24
    ...
    DD(hour, 1) = length(find(hour_grid(lightning_index) > prec_threshold));
...
plot(DD, "b*-")
title_str = strcat("Cell covered with at least ", num2str(lightning_treshold), " and ", num2str(prec_threshold), " mm/hr preceptation");
title(title_str)
xlabel("UT hours")
ylabel("Cell count")
yyaxis right
