function R = plot_8_day(prec_threshold, lightning_treshold, grid_WWLLN_8days, grid_IMERG_8days, VOSTOK_hour_8days, area_mask)
...
%         no_lightning_index=find( grid_WWLLN_8days(:,:,hour,day) < lightning_treshold);
        ...
        DD(hour, 1, day) = sum(sum(area_mask(find(hour_grid(lightning_index) > prec_threshold))));
